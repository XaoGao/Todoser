...
RSpec.describe Tasks::MoveService do
  ...
  describe ".call" do
    let(:task) { create(:task, status: Task.statuses[:doing], position: 1) }

    context "when success" do
      it "result is success" do
        result = service.call(task, Task.statuses[:done], 2, status: Task.statuses[:done], position: 2)
    ...
    context "when failure" do
      it "result is failure" do
        result = service.call(task, "wrong", 2, status: "wrong", position: 2)
